  ...
  return (
    <Fragment>
      <Dropdown drop={Drop} className={Customclass} align={Align} bsPrefix={bsPrefix} as={as} autoClose={autoClose} show={Toggleshow} onToggle={onTogglefunc} onSelect={onSelectfunc}>
        {Buttontag && Buttonposition === "before" ? <SpkButton Buttonvariant={color} Customclass={ButtonCustomClass}>{Buttontext}</SpkButton> : ""}
        <>
          ...
          <Dropdown.Toggle as={toggleas} split={Split} variant={Togglevariant} className={`btn ${Customtoggleclass}`} type="button" href={Navigate} size={Size}
            ...
            {iconPosition === 'before' ?
              (
                <>
                  {Imagetag ? (
                    <img src={Imagesrc} alt="" className={Imageclass} />) : <>{Icon ? (<i className={IconClass}></i>) : ""}</>}
                  {Toggletext}
                </>
              )
              : (
          ...
          {Buttontag && Buttonposition === "after" ? (<SpkButton Buttonvariant={color} Customclass={ButtonCustomClass}>{Buttontext}</SpkButton>) : ''}
