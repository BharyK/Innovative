...
import type { Menuitemtype } from './nav';
  ...
  return (
    <Fragment>
      <Link to="#!" className={`side-menu__item ${MenuItems?.selected ? "active" : ""}`}
        ...
        <span className={`${local_varaiable?.dataVerticalStyle == 'doublemenu' ? '' : 'hidden'}`}>
      ...
      ${MenuItems?.dirchange ? "force-left" : ""} `} style={MenuItems?.active ? (local_varaiable.toggled === "double-menu-close" ? { display: "none" } : { display: local_varaiable.dataNavStyle === "icon-hover" ? "none" : "block" }) : { display: "none" }}
      >
        ...
        {MenuItems?.children.map((firstlevel: Menuitemtype, index: number) =>
          ...
          >
            {firstlevel?.type === "link" ?
              <Link to={firstlevel.path} className={`side-menu__item ${firstlevel.customClass} ${firstlevel?.selected ? 'active' : ''}`}>{firstlevel.icon}
