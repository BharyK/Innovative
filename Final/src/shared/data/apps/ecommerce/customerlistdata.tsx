...
const CustomersData = [
  ['SPK001', 'Aria Robinson', face2, 'Block', '973-277-6950', 'Mar 12, 2025', 'India'],
  ['SPK002', 'John Henry', face3, 'Active', '312-944-1448', 'Mar 10, 2025', 'USA'],
  ['SPK003', 'Mortal Yun', face4, 'Block', '231-480-8536', 'Mar 5, 2025', 'Australia'],
  ['SPK004', 'Saiu Sarah', face7, 'Block', '515-395-1069', 'Mar 2, 2025', 'Japan'],
  ['SPK005', 'Suzika Stallone', face1, 'Block', '414-453-5725', 'Feb 28, 2025', 'Russia'],
  ['SPK006', 'John M. Roberts', face7, 'Active', '213-741-4294', 'Feb 25, 2025', 'France'],
  ['SPK007', 'Alex J. Carter', face8, 'Block', '805-447-8398', 'Feb 20, 2025', 'India'],
  ['SPK008', 'Samuel L. Brooks', face1, 'Block', '786-253-9927', 'Feb 18, 2025', 'Iran'],
];
...
const CustomerList: React.FC<Props> = ({ search, statusFilter, sortBy }) => {
  ...
  useEffect(() => {
    ...
    // Search (ID + Name + Country)
    if (search) {
      const s = search.toLowerCase();
      result = result.filter(row =>
        row[0].toLowerCase().includes(s) || // ID
        row[1].toLowerCase().includes(s) || // Name
        row[6].toLowerCase().includes(s)    // Country
      );
    }

    // Filter by Status
    if (statusFilter && statusFilter !== "All") {
      result = result.filter(row => row[3] === statusFilter);
    }
    ...
    else if (sortBy === "Joining Date") {
      result.sort((a, b) => new Date(b[5]).getTime() - new Date(a[5]).getTime());
    }
    else if (sortBy === "Status") {
      result.sort((a, b) => a[3].localeCompare(b[3]));
    }

    setFilteredData(result);
  }, [search, statusFilter, sortBy, customerData]);
  ...
  useEffect(() => {
    ...
    const grid = new Grid({
      columns: [
        ...
        {
          name: 'Customer ID',
          formatter: (_, row) => html(`<a href="#">${row.cells[0].data}</a>`)
        },
        {
          name: 'Customer Name',
        ...
        },
        {
          name: 'Status',
          formatter: (_, row) =>
            html(`<span class="badge bg-${row.cells[3].data === 'Active' ? 'secondary' : 'danger'}-transparent">${row.cells[3].data}</span>`)
        },
        'Phone',
        'Joining Date',
        {
  ...
  useEffect(() => {
    const handleDelete = (e: ChangeEvent<HTMLButtonElement>) => {
...
export const Customerlistcarddata = [
  {
    id: '1',
    title: 'Total Customers',
    text: '12,754',
    avatarclor: 'primary',
    svgIcon: `<svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 24 24"
